...
export async function getPokemon({ query, page = 1, limit = 1000 }: { query?: string; page?: number; limit?: number }) {
  const apiUrl = `${BASE_URL}/pokemon?limit=${limit}&offset=${(page - 1) * 24}`;
